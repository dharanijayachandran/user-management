...
@Component({
  selector: 'app-dashboard-data-access',
  templateUrl: './dashboard-data-access.component.html',
  styleUrls: ['./dashboard-data-access.component.css']
})
export class DashboardDataAccessComponent implements OnInit {
  ...
  dashboardMap = new Map<number, DataAccessDetail[]>();
  accessTypesMap = new Map<String, Number>();
  saveButtonDisableStatus = false;
  checkedDashboardIds = new Set();
  ...
  getDataAccess(accessGroupId) {
    this.checkedDashboardIds.clear();
    this.dashboardMap.clear();
    this.selectedAccessTypesLength.clear();
    let userId = Number(sessionStorage.getItem("userId"));
    let organizationId = Number(sessionStorage.getItem("beId"));
    this.dataAccessService.getDashboardDataAccessList(organizationId, accessGroupId, userId).subscribe(
      res => {
  ...
  masterIsSelectionToggle(event, row) {
    if (event.checked) {
      row.isSelected = true;
      if (!this.checkedDashboardIds.has(row.entityId) && !this.selectedAccessTypesLength.has(row.entityId)) {
        this.checkedDashboardIds.add(row.entityId);
      }
      if (!this.dashboardMap.has(row.entityId)) {
        this.dashboardMap.set(row.entityId, []);
      }
      this.selection.select(row)
    }
    else {
      row.isSelected = false;
      if (this.checkedDashboardIds.has(row.entityId)) {
        this.checkedDashboardIds.delete(row.entityId);
      }
      if (this.dashboardMap.has(row.entityId)) {
        this.dashboardMap.delete(row.entityId);
      }
  ...
  allAccessTypesDeselectedValidaion() {
    this.dashboardMap.forEach((dataAccessDetail: DataAccessDetail[], entityId: Number) => {
      if (dataAccessDetail.length === 0) {
        this.checkedDashboardIds.add(entityId);
  ...
  addOrRemoveDataAccessValidation(checked, row) {
    let dataAccessDetialList = row.dataAccessDetail.filter((e) => e.accessTypeId != null);
    this.dashboardMap.set(row.entityId, dataAccessDetialList);
  ...
  checkValidation(status, node) {
    if (status == true) {
      if (!this.checkedDashboardIds.has(node.entityId)) {
        if (node.dataAccessDetail) {
          ...
          if (dataAccessDetailsLength <= 0) {
            this.checkedDashboardIds.add(node.entityId);
          }
        }
        else {
          this.checkedDashboardIds.add(node.entityId);
      ...
      if (this.checkedDashboardIds.has(node.entityId)) {
        this.checkedDashboardIds.delete(node.entityId);
      }
    }
  }
