...
@Component({
  selector: 'app-gateway-data-access',
  templateUrl: './gateway-data-access.component.html',
  styleUrls: ['./gateway-data-access.component.css']
})
export class GatewayDataAccessComponent implements OnInit {
  ...
  gatewayMap = new Map<number, DataAccessDetail[]>();
  accessTypesMap = new Map<String, Number>();
  saveButtonDisableStatus = false;
  checkedGatewayIds = new Set();
  ...
  constructor(private formBuilder: FormBuilder, private router: Router, private route: ActivatedRoute, private dataAccessService: DataAccessService, private masterservice: MasterService, private accessGroupService: AccessGroupService, private globalService: globalSharedService) {
  ...
  getDataAccess(accessGroupId) {
    ...
    this.dataAccessService.getGatewayDataAccessList(organizationId, accessGroupId, userId).subscribe(
      res => {
  ...
  masterIsSelectionToggle(event, row) {
    if (event.checked) {
      row.isSelected = true;
      if (!this.checkedGatewayIds.has(row.entityId) && !this.selectedAccessTypesLength.has(row.entityId)) {
        this.checkedGatewayIds.add(row.entityId);
      }
      if (!this.gatewayMap.has(row.entityId)) {
        this.gatewayMap.set(row.entityId, []);
      }
      this.selection.select(row)
    }
    else {
      row.isSelected = false;
      if (this.checkedGatewayIds.has(row.entityId)) {
        this.checkedGatewayIds.delete(row.entityId);
      }
      if (this.gatewayMap.has(row.entityId)) {
        this.gatewayMap.delete(row.entityId);
      }
  ...
  allAccessTypesDeselectedValidaion() {
    this.gatewayMap.forEach((dataAccessDetail: DataAccessDetail[], entityId: Number) => {
      if (dataAccessDetail.length === 0) {
        this.checkedGatewayIds.add(entityId);
  ...
  addOrRemoveDataAccessValidation(checked, row) {
    let dataAccessDetialList = row.dataAccessDetail.filter((e) => e.accessTypeId != null);
    this.gatewayMap.set(row.entityId, dataAccessDetialList);
  ...
  checkValidation(status, node) {
    if (status == true) {
      if (!this.checkedGatewayIds.has(node.entityId)) {
        if (node.dataAccessDetail) {
          ...
          if (dataAccessDetailsLength <= 0) {
            this.checkedGatewayIds.add(node.entityId);
          }
        }
        else {
          this.checkedGatewayIds.add(node.entityId);
      ...
      if (this.checkedGatewayIds.has(node.entityId)) {
        this.checkedGatewayIds.delete(node.entityId);
      }
    }
  }
